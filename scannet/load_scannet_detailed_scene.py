...
def read_aggregation(filename):
    ...
    with open(filename) as f:
        ...
        for i in range(num_objects):
            ...
            object_id_to_label_name[object_id] = label
            if label in label_to_segs:
                label_to_segs[label].extend(segs)
            else:
                label_to_segs[label] = segs
    return object_id_to_segs, label_to_segs, object_id_to_label_name
...
def export(mesh_file, agg_file, seg_file, meta_file, label_map_file, output_file=None):
    ...
    category_id_map = scannet_utils.read_label_mapping(label_map_file, label_from='raw_category', label_to='id')
    nyu40class_map = scannet_utils.read_label_mapping(label_map_file, label_from='nyu40class', label_to='nyu40id')
    # id2category_map = scannet_utils.read_label_mapping(label_map_file, label_from='id', label_to='raw_category')
    id2category_map = {v: k for k, v in category_id_map.items()}
    nyu40id2class_map = {v: k for k, v in nyu40class_map.items()}
    ...
    if os.path.isfile(agg_file):
        object_id_to_segs, label_to_segs, object_id_to_label_name = read_aggregation(agg_file)
        seg_to_verts, num_verts = read_segmentation(seg_file)

        label_ids = np.zeros(shape=(num_verts), dtype=np.uint32) # 0: unannotated
        category_ids = np.zeros(shape=(num_verts), dtype=np.uint32) # 0: unannotated
        object_id_to_label_id = {}
        object_id_to_category = {}
        for label, segs in label_to_segs.items():
            label_id = label_map[label] # label is raw category
            category_id = category_id_map[label]
            for seg in segs:
                verts = seg_to_verts[seg]
                label_ids[verts] = label_id
                category_ids[verts] = category_id
        ...
        for object_id, segs in object_id_to_segs.items():
            for seg in segs:
                ...
                if object_id not in object_id_to_label_id:
                    ...
                    # object_id_to_category[object_id] = id2category_map[category_ids[verts][0]] # may lead to duplicate id for different categories (e.g., door and doors; picture and pictures)
                    object_id_to_category[object_id] = object_id_to_label_name[object_id]
        ...
        for obj_id in object_id_to_segs:
            ...
            nyu40class = nyu40id2class_map[label_id]
            nyu40classes.append(nyu40class)
    ...
    if output_file is not None:
        ...
        # np.save(output_file+'_aligned_bbox.npy', instance_bboxes)  # wrong?
        np.save(output_file+'_aligned_bbox.npy', aligned_instance_bboxes)
        json.dump(categories, open(output_file+'_categories.json', 'w'))
        json.dump(nyu40classes, open(output_file+'_nyu40classes.json', 'w'))
